    ...
    Plane(float sizeX = 10, float sizeY = 10, unsigned int resolutionX = 32, unsigned int resolutionY = 32){
        for(unsigned int i = 0; i<resolutionX; i++){
            for(unsigned int j = 0; j<resolutionY; j++){
                vertices.push_back(Vertex(((float)i/(resolutionX-1)-0.5)*sizeX, 0, ((float)i/(resolutionY-1)-0.5)*sizeY));
        ...
        for(unsigned int i = 0; i<resolutionX; i++){
            for(unsigned int j = 0; j<resolutionY; j++){
                if(i<resolutionX-1 && j<resolutionY-1){
                    ...
                    Triangle t1; t1.corners[0] = i; t1.corners[1] = voisin_droite; t1.corners[2] = voisin_haut_droite;
                    Triangle t2; t2.corners[0] = i; t2.corners[1] = voisin_haut; t2.corners[2] = voisin_haut_droite;
